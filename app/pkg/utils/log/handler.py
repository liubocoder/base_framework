...
class InterceptTimedRotatingFileHandler(logging.Handler):
    ...
    缺少命名空间
    """
    def __init__(self, filename, when='', interval=1, backup_count=15, encoding="utf-8", delay=False, utc=False,
                 atTime=None, logging_levels="all", level=logging.NOTSET):
        ...
        retention = "%d %ss" % (backup_count, key_map[when])
        # if when == "s":
